...
void state_idle_entry(void) {
    motor_stop();
    // LED1 không du?c t?t ? IDLE - ch? t?t khi system OFF ho?c END hoàn thành
    run_flag = FALSE;
}
...
void state_idle_exit(void) {
    // LED1 dã sáng t? khi b?t ngu?n - không c?n b?t l?i
}
...
void state_end_reverse_update(void) {
    ...
    // Ki?m tra n?u c? 3 c?m bi?n th?y n?n tr?ng (000)
    if (pattern == 0b000) {
        // D?ng motor
        motor_stop();
        motor_disable();
        ...
        // Ch? m?t chút tru?c khi reset
        delay_ms(1000);
        
        // Reset h? th?ng d? b?t d?u chu k? m?i
        reset_cpu();
    }
    
    // Ti?p t?c lùi n?u chua th?y n?n tr?ng
    // (motor dã du?c b?t trong entry)
}
...
void state_error_entry(void) {
    motor_stop();
    // LED1 không t?t - ch? nháy d? báo l?i
}
...
void state_error_exit(void) {
    // Ð?m b?o LED1 sáng khi thoát kh?i error state
    sensors_led_status(TRUE);
}
