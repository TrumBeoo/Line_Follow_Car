...
void timer0_isr(void) {
    ...
    // Duy trì tr?ng thái LED1 - luôn sáng khi system enabled
    ...
    // Handle LED2 control based on middle sensor - CH? KHI SYSTEM ENABLED VÀ KHÔNG BLINK
    if (system_enabled && !led2_blink_enable) {
        int8 current_pattern = sensors_read_filtered();
        // LED2 t?t ch? khi sensor gi?a th?y line den (bit 1 = 1)
    ...
    if (led_tick >= 125) {  // 125 * 2ms = 250ms
        ...
        // Handle LED2 blinking when enabled - UU TIÊN BLINK H?N SENSOR LOGIC
        if (led2_blink_enable) {
            ...
            if (led2_blink_count >= 6) {
                ...
                led2_blink_count = 0;
                // Sau khi blink xong, tr? v? logic sensor
                if (system_enabled) {
                    int8 current_pattern = sensors_read_filtered();
                    if (current_pattern & 0b010) {
...
volatile int1 cp1_long_press = FALSE;      // Flag nh?n gi? STOP
volatile int32 stop_press_time = 0;        // Th?i gian b?t d?u nh?n STOP
volatile int1 stop_is_pressed = FALSE;     // Tr?ng thái nh?n STOP
...
void portb_change_isr(void) {
    ...
    if (!input(BUTTON_SYSTEM)) {  // Active low - dang nh?n
        ...
        if (!input(BUTTON_SYSTEM)) {
            ...
            if (!system_enabled) {
                ...
                // System ON - c? 2 LED sáng khi b?t ngu?n
    ...
    // STOP button (B0) - x? lý nh?n gi?/th? cho checkpoint logic
    if (!input(BUTTON_STOP)) {  // Active low - dang nh?n
        if (!stop_is_pressed) {
        ...
        if (stop_is_pressed) {
            ...
            if (press_duration >= 1000) {  // Nh?n gi? >= 1s
                stop_pressed = TRUE;
                cp1_long_press = TRUE;
            } else {  // Nh?n th? < 1s
                stop_pressed = TRUE;
...
void handle_checkpoint_buttons(void) {
    ...
    // X? lý nút STOP (B0) - checkpoint logic
    if (stop_pressed) {
        stop_pressed = FALSE;
        
        if (cp1_long_press) {  // Nh?n gi? >= 1s = STOP function
            // M?c d?nh v? checkpoint 1, chua có bi
        ...
        } else {  // Nh?n th? < 1s = Cycle through checkpoints
            // Chuy?n d?i gi?a các checkpoint
            switch (current_checkpoint) {
                case CP_START:
                    // Checkpoint 1 v?i bi dã l?y
                    current_checkpoint = CP_START;
                    ball_grabbed = TRUE;
                    // LED2 OFF - ch? nháy khi l?y bi thành công t?i station
                    sensors_led_action(FALSE);
                    break;
                    
                case CP_AFTER_NAVIGATION:
                    // Chuy?n sang checkpoint 3
            ...
            // Không t? d?ng chuy?n state, ch? RUN button
        }
    }
    ...
    if (run_pressed) {
        ...
        // Th?c hi?n RUN t? checkpoint hi?n t?i
        switch (current_checkpoint) {
            case CP_START:
                if (ball_grabbed) {
                    // Ðã có bi - b? qua station, th?ng navigation
                    fsm_transition(STATE_FOLLOW_LINE);
                } else {
                    // Chua có bi - bám line và vào station
                    fsm_transition(STATE_FOLLOW_LINE);
                }
                break;
                
            case CP_AFTER_NAVIGATION:
                // Checkpoint 2 - dã có bi, ti?p t?c bám line
                ball_grabbed = TRUE;
...
void main(void) {
    ...
    led1_should_be_on = FALSE;  // LED1 ban d?u t?t
